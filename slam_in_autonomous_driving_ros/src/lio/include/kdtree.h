...
class KdTree {
   public:
    ...
    bool BuildTree(const CloudPtr& cloud);      // 使用递归构建kdtree
    bool BuildTree_for(const CloudPtr& cloud);  // 使用for循环构建kdtree
   ...
   private:
    ...
    void Insertfor(KdTreeNode* node);
    ...
    std::list<std::pair<IndexVec,KdTreeNode*>> points_and_nodes_;// 需要遍历的点云索引组和对应的节点组成的列表
