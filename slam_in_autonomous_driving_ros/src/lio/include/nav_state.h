...
struct NavState {
    ...
    explicit NavState(const nav_msgs::Odometry& odom) {
        Eigen::Quaterniond q(odom.pose.pose.orientation.w, odom.pose.pose.orientation.x,
                              odom.pose.pose.orientation.y, odom.pose.pose.orientation.z);
        timestamp_ = odom.header.stamp.toSec();
        R_ = SO3(q);
        p_ <<odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z;
        v_ <<odom.twist.twist.linear.x, odom.twist.twist.linear.y, odom.twist.twist.linear.z;
    }
