...
bool KdTree::BuildTree_for(const CloudPtr &cloud) 
{
    if (cloud->empty()) {
        return false;
    }
    ...
    IndexVec idx(cloud->size());//存储初始点云索引组
    for (int i = 0; i < cloud->points.size(); ++i) {
        idx[i] = i;
    }

    std::pair<IndexVec,KdTreeNode*> pair0(idx, root_.get()); //pair0.first为点云索引组，pair0.second为根节点
    points_and_nodes_.emplace_front(pair0);//在容器的头部插入初始点云索引组和根节点

    while (!points_and_nodes_.empty()) {

        auto points_current_    = points_and_nodes_.front().first;  //取出容器头部的点云索引组
        auto root_current_      = points_and_nodes_.front().second; //取出容器头部的节点

        points_and_nodes_.pop_front();//删除容器头部的点云索引组

        //初始化根节点的id经过Reset()函数后为0，
        //后续节点的id默认初始化为-1 （line 120 和 line125）
        // 经过tree_node_id_++，后续节点为1,2,3,4,...,n
        if(root_current_->id_ == -1){
            root_current_->id_ = tree_node_id_++;
        }
        // 点云索引组为空，跳过
        if (points_current_.empty()) {
            continue;
        }
        // 点云索引组只有一个点，直接插入
        if (points_current_.size() == 1) {
            size_++;//节点数量加1
            root_current_->point_idx_ = points_current_[0];//将当前节点的索引赋值为点云索引
            Insertfor(root_current_);//将节点插入节点容器
            continue;
        }

        IndexVec left, right;//存储左右子树的点云索引组
        if (!FindSplitAxisAndThresh(points_current_, root_current_->axis_index_, root_current_->split_thresh_, left, right)) {
            size_++;
            root_current_->point_idx_ = points_current_[0];//输入的points等于同一个值，这种情况不需要继续展开，直接将当前节点设为叶子就行
            Insertfor(root_current_);//将节点插入节点容器
            continue;
        }

        if(!left.empty()){
            root_current_->left_ = new KdTreeNode;//创建左子树节点
            std::pair<IndexVec,KdTreeNode*> pair_left(left, root_current_->left_);//创建左子树节点和点云索引组的pair
            points_and_nodes_.emplace(points_and_nodes_.begin(), pair_left);//在容器的头部插入左子树节点和点云索引组的pair
        }
        if(!right.empty()){
            root_current_->right_ = new KdTreeNode;
            std::pair<IndexVec,KdTreeNode*> pair_right(right, root_current_->right_);
            points_and_nodes_.emplace(++points_and_nodes_.begin(), pair_right);//在容器的第二个位置插入右子树节点和点云索引组的pair
        }

        Insertfor(root_current_);//将根节点插入节点容器

        }
}
void KdTree::Insertfor(KdTreeNode* node)
{
    nodes_.insert({node->id_, node});
