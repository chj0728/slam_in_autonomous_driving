...
GMapping::GMapping(): private_node_("~"),
                    ...
                    gmapping_map_(center, xmin_, ymin_, xmax_, ymax_, resolution_)
...
{   
    ...
    // 参数初始化
    InitParams();
    gmapping_map_.resize(xmin_, ymin_, xmax_, ymax_);

    node_handle_.param<std::string>("map_frame", map_frame_, "map");

    laser_scan_subscriber_  = node_handle_.subscribe("scan_multi", 1, &GMapping::ScanCallback, this);
...
void GMapping::PublishMap(const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
    ...
    //根据激光雷达的位姿更新地图的大小
    // *******************************************************************************************
    geometry_msgs::TransformStamped laser_pose_in_map;
    try
    {
        laser_pose_in_map = tf_buffer_.lookupTransform(map_frame_, scan_msg->header.frame_id, ros::Time(0), ros::Duration(0.1));
    }
    ...
    // 地图图片像素的大小, width为地图的宽度是多少个像素
    if( xmax_ -laser_pose_in_map.transform.translation.x < 25 || 
        ymax_ - laser_pose_in_map.transform.translation.y < 25 || 
        laser_pose_in_map.transform.translation.x - xmin_ < 25 || 
        laser_pose_in_map.transform.translation.y - ymin_ < 25)
    {
        ymin_ -= 10;
        xmax_ += 10;
        ymax_ += 10;
...
void GMapping::ComputeMap(ScanMatcherMap &map, const sensor_msgs::LaserScan::ConstPtr &scan_msg)
{
    ...
    tf2::Quaternion q_;
    tf2::fromMsg(laser_pose_in_map.transform.rotation, q_);
    double roll, pitch, yaw;
    tf2::Matrix3x3(q_).getRPY(roll, pitch, yaw);

    // gmapping_map_.resize(   xmin_ + laser_pose_in_map.transform.translation.x, 
    //                         ymin_ + laser_pose_in_map.transform.translation.y, 
    //                         xmax_ + laser_pose_in_map.transform.translation.x, 
    //                         ymax_ + laser_pose_in_map.transform.translation.y);
    // gmapping_map_.resize(xmin_, ymin_, xmax_, ymax_);
    // gmapping_map_.resize(-50, -50, 50 , 50);


    // lp为地图坐标系下的激光雷达坐标系的位姿
    // OrientedPoint lp(0, 0, 0.0);
    OrientedPoint lp(   laser_pose_in_map.transform.translation.x, 
                        laser_pose_in_map.transform.translation.y, 
                        yaw);
